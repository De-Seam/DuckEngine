...
namespace fm //Fast Math
{
	...
	struct vec2
	{
		...
		vec2 operator - (const vec2& i) const { return { x - i.x, y - i.y }; }
		vec2 operator * (const vec2& i) const { return { x * i.x, y * i.y }; }
		vec2 operator * (const f32 i) const { return { x * i, y * i }; }
		vec2 operator / (const vec2& i) const { return { x / i.x, y / i.y }; }
		vec2 operator / (const f32 i) const { return { x / i, y / i }; }
		const vec2& operator += (const vec2& i) { return *this = *this + i; }
		const vec2& operator -= (const vec2& i) { return *this = *this - i; }
		const vec2& operator *= (const vec2& i) { return *this = *this * i; }
		const vec2& operator *= (const f32 i) { return *this = *this * i; }
		const vec2& operator /= (const vec2& i) { return *this = *this / i; }
		const vec2& operator /= (const f32 i) { return *this = *this / i; }
	};
	...
	struct vec3
	{
		...
		vec3 operator - (const vec3& i) const { return { x - i.x, y - i.y, z - i.z }; }
		vec3 operator * (const vec3& i) const { return { x * i.x, y * i.y, z * i.z }; }
		vec3 operator * (const f32 i) const { return { x * i, y * i, z * i }; }
		vec3 operator / (const vec3& i) const { return { x / i.x, y / i.y, z / i.z }; }
		vec3 operator / (const f32 i) const { return { x / i, y / i, z / i }; }
		const vec3& operator += (const vec3& i) { return *this = *this + i; }
		const vec3& operator -= (const vec3& i) { return *this = *this - i; }
		const vec3& operator *= (const vec3& i) { return *this = *this * i; }
		const vec3& operator *= (const f32 i) { return *this = *this * i; }
		const vec3& operator /= (const vec3& i) { return *this = *this / i; }
		const vec3& operator /= (const f32 i) { return *this = *this / i; }

		bool operator==(const vec3& i) { return x == i.x && y == i.y && z == i.z;}

		[[nodiscard]] u32 get_argb() const
		{
			const f32 t_r = clamp<f32>(x, 0, 1);
			const f32 t_g = clamp<f32>(y, 0, 1);
			const f32 t_b = clamp<f32>(z, 0, 1);
			return 255 | (static_cast<u8>(t_r * 255) << 16) | (static_cast<u8>(t_g * 255) << 8) | (static_cast<u8>(t_b * 255));
		}

		[[nodiscard]] u32 get_rgba() const
		{
			const f32 t_r = clamp<f32>(x, 0, 1);
			const f32 t_g = clamp<f32>(y, 0, 1);
			const f32 t_b = clamp<f32>(z, 0, 1);
			return static_cast<u32>((static_cast<u8>(t_r * 255) << 24) | (static_cast<u8>(t_g * 255) << 16) | (static_cast<u8>(t_b * 255) << 8) | 255);
		}

		[[nodiscard]] f32 dot(const vec3& i) const
		{
	...
	struct alignas(4 * sizeof(f32)) vec4
	{
		...
		vec4 operator - (const vec4& i) const { return { x - i.x, y - i.y, z - i.z, w - i.w }; }
		vec4 operator * (const vec4& i) const { return { x * i.x, y * i.y, z * i.z, w * i.w }; }
		vec4 operator * (const f32 i) const { return { x * i, y * i, z * i, w * i }; }
		vec4 operator / (const vec4& i) const { return { x / i.x, y / i.y, z / i.z, w / i.w }; }
		vec4 operator / (const f32 i) const { return { x / i, y / i, z / i, w / i }; }
		const vec4& operator += (const vec4& i) { return *this = *this + i; }
		const vec4& operator -= (const vec4& i) { return *this = *this - i; }
		const vec4& operator *= (const vec4& i) { return *this = *this * i; }
		const vec4& operator *= (const f32 i) { return *this = *this * i; }
		const vec4& operator /= (const vec4& i) { return *this = *this / i; }
		const vec4& operator /= (const f32 i) { return *this = *this / i; }

		[[nodiscard]] u32 get_argb() const
		{
			const f32 t_a = clamp<f32>(w, 0, 1);
			const f32 t_r = clamp<f32>(x, 0, 1);
			const f32 t_g = clamp<f32>(y, 0, 1);
			const f32 t_b = clamp<f32>(z, 0, 1);
			return static_cast<u32>((static_cast<u8>(t_a * 255) << 24) | (static_cast<u8>(t_r * 255) << 16) | (static_cast<u8>(t_g * 255) << 8) | (static_cast<u8>(t_b * 255)));
		}
		...
		[[nodiscard]] u32 get_rgba() const
		{
			...
			const f32 t_g = clamp<f32>(y, 0, 1);
			const f32 t_b = clamp<f32>(z, 0, 1);
			return static_cast<u32>((static_cast<u8>(t_r * 255) << 24) | (static_cast<uint8_t>(t_g * 255) << 16) | (static_cast<u8>(t_b * 255) << 8) | (static_cast<u8>(t_a * 255)));
		}
	};

	inline f32 get_m128_together(__m128 i)
	{
		f32 result[4];
	...
	struct quat
	{
		...
		quat(f32 x, f32 y, f32 z, f32 w)
			: x(x), y(y), z(z), w(w){}
		quat(f32 a_x, f32 a_y, f32 a_z)
		{
			f32 cy = std::cosf(a_z * 0.5f);
			f32 sy = std::sinf(a_z * 0.5f);
			f32 cp = std::cosf(a_y * 0.5f);
			f32 sp = std::sinf(a_y * 0.5f);
			f32 cr = std::cosf(a_x * 0.5f);
			f32 sr = std::sinf(a_x * 0.5f);
		...
		quat operator - (const quat& i) const { return { x - i.x, y - i.y, z - i.z, w - i.w }; }
		quat operator * (const f32 i) const { return { x * i, y * i, z * i, w * i }; }
		...
		const quat& operator -= (const quat& i) { return *this = *this - i; }
		const quat& operator *= (const quat& i) { return *this = *this * i; }
		const quat& operator *= (const f32 i) { return *this = *this * i; }
	...
	{
		f32 mag = std::sqrtf(i.x * i.x + i.y * i.y);
	...
	{
		f32 mag = std::sqrtf(i.x * i.x + i.y * i.y + i.z * i.z);
		assert(mag != 0.f && "Try using normalize_safe instead");
		f32 inv = 1.f / mag;
	...
	inline mat4 get_perspective(f32 fovy, f32 aspect, f32 znear, f32 zfar)
	{
		assert(abs(aspect - std::numeric_limits<f32>::epsilon()) > 0.f);

		f32 const tanHalfFovy = tan(fovy / 2.f);
	...
	{
		f32 n = sqrtf(q.x * q.x + q.y * q.y + q.z * q.z + q.w * q.w);
		if (n <= 0.f)
			return { 0,0,0,0 };
		n = 1.f / n;
		return
	...
	inline quat angle_axis(f32 angle, const vec3& i)
	{
		f32 a(angle);
	...
	inline f32 wang_hashf(u32 seed) // Return value between 0.f amd 1.f
	{
		return wang_hash(seed) * (1.0f / 4294967296.0f);
	}

	inline u32 xorshift32(u32* pstate)
	{
	...
	inline f32 xorshift32f(u32* state) // Return value between 0.f amd 1.f
	{
