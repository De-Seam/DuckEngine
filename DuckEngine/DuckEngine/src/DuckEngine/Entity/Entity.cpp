...
void Entity::Draw()
{
	if (!m_texture)
	{
		Renderer::DrawRectangleOutline(m_position, m_halfSize, {1, 1, 1, 1});
		return;
	}
	...
	Renderer::DrawTexture(texture, m_position, m_halfSize, m_rotation);
}
...
void Entity::SetJSONVariables(nlohmann::json json)
{
	...
	SetHalfSize({json["size"][0], json["size"][1]});
...
nlohmann::json Entity::GetJSONVariables() const
{
	...
	json["size"] = {m_halfSize.x, m_halfSize.y};
